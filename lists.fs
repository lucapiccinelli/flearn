...
let evens = [2; 4; 6; 8]
let cento = [0..10]
let doubleCento = List.map (fun x -> x * 2) cento
let filteredDoubledCento = List.map (fun x -> x * 2) (List.filter (fun x -> x % 2 = 0) [0..100])
let bla = List.sum( List.map (fun x -> x * 2) ( List.filter (fun x -> x > 3) [0..5] ) )
